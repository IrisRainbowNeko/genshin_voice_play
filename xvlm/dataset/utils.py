...
def segmentation_eval(results, maskUtils, refer, alpha, mask_size=12):
    num_A, num_B, num_val = 0, 0, 0
    iou_A_s, iou_B_s, iou_val_s = 0, 0, 0
    ...
    for res in tqdm(results):
        ...
        mask = torch.where(mask>alpha,1,0)

        seg = refer.refToAnn[ref_id]['segmentation']
        rle = maskUtils.frPyObjects(seg, image['height'], image['width'])
        gt_mask = torch.from_numpy(maskUtils.decode(rle)[:,:,0]).cuda()

        iou = torch.min(mask,gt_mask).count_nonzero()/torch.max(mask,gt_mask).count_nonzero()
        ...
        if ref['split'] == 'testA':
            ...
            num_val += 1
            iou_val_s += iou
    iou_val_s,iou_A_s,iou_B_s = map(lambda x:x.cpu().detach().item(),[iou_val_s,iou_A_s,iou_B_s])
    
    eval_result = {'val_s': iou_val_s / num_val, 'testA_s': iou_A_s / num_A, 'testB_s': iou_B_s / num_B}
...
def genshin_eval_bbox(results, refer):
    correct = 0
    num = 0
